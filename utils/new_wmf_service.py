...
class Service():
    ...
    def __str__(self):
        return '{0!s}'.format(self.service_name)

    def __unicode__(self):
        return u'{0!s}'.format(self.__str__())

    def create_puppet_module(self):
        # Create directories
        try:
            os.makedirs('modules/{0!s}/manifests/'.format(self.service_name))
            os.makedirs('modules/{0!s}/tests/'.format(self.service_name))
            os.makedirs('modules/{0!s}/templates/'.format(self.service_name))
        except OSError as e:
            print 'Can not create directories. Error: {0!s}'.format(e)
            return False

        # Populate tests
        with open('modules/{0!s}/tests/init.pp'.format(self.service_name), 'w') as f:
            f.write('include ::{0!s}\n'.format(self.service_name))

        with open('modules/{0!s}/tests/Makefile'.format(self.service_name), 'w') as f:
...
''')
        ...
        # Populate templates
        with open('modules/{0!s}/templates/config.yaml.erb'.format(self.service_name), 'w') as f:
            f.write('# Generated by new_wmf_service.py.\n{}\n')

        # Populate manifests
        if self.service_node:
            with open('modules/{0!s}/manifests/init.pp'.format(self.service_name), 'w') as f:
                f.write('''
# Class: {name!s}
#
...
'''.format(**{'name': self.service_name, 'port': self.port}))
        return True

    def create_puppet_role(self):
        # Populate role
        if self.service_node:
            with open('manifests/role/{0!s}.pp'.format(self.service_name), 'w') as f:
                f.write('''
...
'''.format(**{'name': self.service_name, 'description': self.service_description}))
        return True
    ...
    def create_deployment_config(self):
        ...
        # Add our repo
        repos['repo_config']['{0!s}/deploy'.format(self.service_name)] = {
            'upstream': self.repo,
        }
    ...
    def assign_service_to_cluster(self):
        new = []
        with open('manifests/role/{0!s}.pp'.format(self.cluster), 'r') as f:
            old = f.readlines()
        ...
        for line in old:
            ...
            if match:
                ...
                new.append('{0!s}include role::{1!s}\n'.format(indent, self.service_name))
                inroles = False
            else:
                inroles = False
            new.append(line)
        with open('manifests/role/{0!s}.pp'.format(self.cluster), 'w') as f:
            f.writelines(new)
        return True

    def setup_lvs_ip(self):
        with open('hieradata/role/common/{0!s}.yaml'.format(self.cluster), 'r') as f:
            data = f.read()
        ...
        with open('hieradata/role/common/{0!s}.yaml'.format(self.cluster), 'w') as f:
            f.writelines(data)
        return True
    ...
    def setup_lvs(self):
        ...
        config['lvs_services'][self.service_name] = {
            ...
            'icinga': {
                'check_command': ('check_http_lvs_on_port!{0!s}!{1!s}!/_info'.format(self.lvs_hostname, self.port)),
                'sites': {
                    'eqiad': {
    ...
    def setup_accounts(self):
        ...
        config = ordered_load(data)
        # Add our new group
        config['admin::groups'].append('{0!s}-admin'.format(self.service_name))
        data = ordered_dump(config, default_flow_style=False)
        with open('hieradata/role/common/{0!s}.yaml'.format(self.cluster), 'w') as f:
            f.writelines(data)
        return True
    ...
    def setup_sudo_rights(self):
        ...
        newgroup = {
            'gid': gid,
            'description': 'Group of {0!s} admins'.format(self.service_name),
            'members': [self.point_person],
            'privileges': [
                'ALL = NOPASSWD: /usr/sbin/service {0!s} *'.format(self.service_name),
                'ALL = ({0!s}) NOPASSWD: ALL'.format(self.service_name),
            ]
        }
        groups['{0!s}-admin'.format(self.service_name)] = newgroup
...
def question_user(answers):
    for q in QUESTIONS:
        ...
        while not successful:
            answer = raw_input('{0!s}? '.format(q['qstring']))
...
def main():
    ...
    if not s.create_puppet_module():
        ...
        return False
    Git.add_file('modules/{0!s}'.format(s.service_name))
    if not s.create_puppet_role():
        print 'Failed to create puppet role'
        return False
    Git.add_file('manifests/role/{0!s}.pp'.format(s.service_name))
    ...
    Git.commit('Introducing {0!s} role and puppet module'.format(s.service_name))
    if not s.assign_service_to_cluster():
        print 'Failed to assign role to cluster'
        return False
    Git.add_file('manifests/role/{0!s}.pp'.format(s.cluster))
    if not s.setup_accounts():
        print 'Failed to setup accounts'
        return False
    Git.add_file('hieradata/role/common/{0!s}.yaml'.format(s.cluster))
    if not s.setup_sudo_rights():
        print 'Failed to setup accounts'
        return False
    Git.add_file('modules/admin/data/data.yaml')
    # Let's commit the second batch
    Git.commit('Assign {0!s} service to {1!s} cluster'.format(s.service_name, s.cluster))
    ...
    Git.commit('Setup LVS for {0!s} service on {1!s} cluster'.format(s.service_name, s.cluster))
...
def handle_args():
    ...
    for q in QUESTIONS:
        parser.add_argument('--{0!s}'.format(q['qname']),
                            help=q['qstring'],
                            action='store',
                            dest='{0!s}'.format(q['qname']))
