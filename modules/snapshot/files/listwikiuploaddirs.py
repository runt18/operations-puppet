...
class UploadDir(object):
    ...
    def get_upload_dir(self, wiki):
        ...
        want."""
        input_text = ("global $wgupload_directory; "
                      "echo \"$wgupload_directory\";")
...
def usage(message=None):
    ...
    usagemessage = """Usage: python listwikiuploaddirs.py --allwikis filename
                                   --scriptpath dir
                                   [multiversion] [closedwikis filename]
                                   [privatewikis filename] [skipwikis filename]
                                   [wmfhack]
...
$wgupload_directory unless the 'wmfhack' option is specified, see below
for that
...
def main():
    ...
    script_path = None
    wmfhack = False
    all_wikis_file = closed_wikis_file = None
    private_wikis_file = skip_wikis_file = None

    try:
        (options, rem) = getopt.gnu_getopt(
            sys.argv[1:], "", ["allwikis=", "closedwikis=",
