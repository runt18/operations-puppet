...
def build_dmblock_major_minor_tables():
    ...
    3) a dict mapping each (major,minor) pair to a table of devce mapper
       names
    """
...
def build_block_major_minor_tables():
    ...
    p = subprocess.Popen("awk '{print $1,$2, $3}' " + DISKSTATS_FILE,
...
def get_partitions():
    ...
    if DEVICES is not None:
        ...
        # Load partitions
        awk_cmd = "awk 'NR > 1 && $0 !~ /" + IGNORE_DEV + \
                  "/ && $4 !~ /[0-9]$/ {ORS=\" \"; print $4}' "
        p = subprocess.Popen(awk_cmd + PARTITIONS_FILE, shell=True,
...
# 5 merge_read    Reads and writes which are adjacent to each other may be
#                 merged for efficiency.  Thus two 4K reads may become one 8K
#                 read before it is ultimately handed to the disk, and so it
#                 will be counted (and queued) as only one I/O.  This field
#                 lets you know how often this was done.
...
# 9 merge_write   Reads and writes which are adjacent to each other may be
#                 merged for efficiency.  Thus two 4K reads may become one 8K
#                 read before it is ultimately handed to the disk, and so it
#                 will be counted (and queued) as only one I/O.  This field
#                 lets you know how often this was done.
...
# 12 ios          The only field that should go to zero. Incremented as
...
# 14 ms_weighted  This field is incremented at each I/O start, I/O completion,
#                 I/O
###########################################################################
def update_stats():
    ...
    if cur_time - last_update < MAX_UPDATE_TIME:
        logging.debug(' wait ' + str(int(MAX_UPDATE_TIME -
    ...
    for dev in PARTITIONS:
        ...
        # Get values from diskstats file
        p = subprocess.Popen(
            "awk -v dev=" + dev + " '$3 == dev' " + DISKSTATS_FILE, shell=True,
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = p.communicate()
        ...
        get_delta(dev, 'read_bytes_per_sec',
                  int(vals[5]), float(BYTES_PER_SECTOR))
        get_delta(dev, 'write_bytes_per_sec',
                  int(vals[9]), float(BYTES_PER_SECTOR))
...
def metric_init(params):
    ...
    descriptions = dict(
        ...
        reads_merged={
            'units': 'reads',
            'description': 'The number of reads merged. Reads which are ' +
                           'adjacent to each other may be merged for ' +
                           'efficiency. Multiple reads may become one ' +
                           'before it is handed to the disk, and it will be ' +
                           'counted (and queued) as only one I/O.'},
        ...
        writes_merged={
            'units': 'writes',
            'description': 'The number of writes merged. Writes which are ' +
                           'adjacent to each other may be merged for ' +
                           'efficiency. Multiple writes may become one ' +
                           'before it is handed to the disk, and it will be ' +
                           'counted (and queued) as only one I/O.'},
        ...
        weighted_io_time={
            'units': 's',
            'description': 'The weighted time in seconds spend in I/O ' +
                           'operations. This measures each I/O start, I/O ' +
                           'completion, I/O merge, or read of these stats ' +
                           'by the number of I/O operations in progress ' +
                           'times the number of seconds spent doing I/O.'}
    )
...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option('-d', '--devices', dest='devices', default='',
                      help='devices to explicitly check')
    parser.add_option('-b', '--gmetric-bin', dest='gmetric_bin',
                      default='/usr/bin/gmetric',
                      help='path to gmetric binary')
    parser.add_option('-c', '--gmond-conf', dest='gmond_conf',
                      default='/etc/ganglia/gmond.conf',
                      help='path to gmond.conf')
    parser.add_option('-g', '--gmetric', dest='gmetric', action='store_true',
                      default=False, help='submit via gmetric')
    parser.add_option('-m', '--device-mapper', dest='device_mapper',
                      action='store_true', default=False,
                      help='utilize all device mapper devices if set')
    ...
    while True:
        for d in descriptors:
            ...
            if options.gmetric:
                ...
                cmd = ("%s --conf=%s --value='%s' --units='%s' --type='%s' " +
                       "--name='%s' --slope='%s'") % \
                      (options.gmetric_bin, options.gmond_conf, v, d['units'],
                       value_type, d['name'], d['slope'])
                os.system(cmd)
