...
class ExtendedEximCollector(diamond.collector.Collector):
    def get_default_config_help(self):
        config_help = super(ExtendedEximCollector, self).get_default_config_help()  # noqa
        config_help.update({
    ...
    def _get_queue(self):
        ...
        # remove empty lines
        queue = [l.rstrip() for l in queue.split("\n")]
    ...
    def collect_queue(self):
        ...
        if parsed_queue:
            oldest = max(l.age for l in parsed_queue).total_seconds()
            youngest = min(l.age for l in parsed_queue).total_seconds()
